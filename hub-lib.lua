...
coreGUIFuncs.newCreateGUI = function(name, pos, parent, colors)
	...
	coreFuncs.addInstance("TextLabel", {
		...
		["Text"] = game.Players.LocalPlayer.Name.."#"..tostring(math.random(1000,9999)),
...
------------------------------------------------------------------------------
-- hublin.create and newNotification Class

coreVars.activeNotification = nil;
coreVars.notificationHolder = Instance.new("ScreenGui", game.CoreGui);
hublin.newNotif = function(notifType, text, buttonLT, buttonRT, buttonLC, buttonRC, theme)
	local notif = {};
	...
	elseif notif.notifType == hublin.enum.notifType.buttons then
		...
		notifInstance.buttonl.MouseButton1Click:Connect(function()
			...
			notif.fireCallback(hublin.enum.button.left);
		end)
		notifInstance.buttonr.MouseButton1Click:Connect(function() 
			notif.buttonRightClicked:Fire();
			notif.buttonClicked:Fire();
			notif.fireCallback(hublin.enum.button.right);
		end)
	else
	...
	notif.fireCallback = function(button)
		if button == hublin.enum.button.left then
			notif.callbackL();
		elseif button == hublin.enum.button.right then
			notif.callbackR();
		end
	end
	...
	notif.play = function(delay)
		...
		playDebounce = true;
		return notif;	-- This is so that you can do something like hublin.newNotif(...).play().delete();
	end
	...
	-- Event handling for notif.buttonClicked - Only applies to hublin.enum.notifType.buttons
...
hublin.create = function(name, parent, pos, theme, gameID, deleteOldGUI)
	local create = {};
	...
	if deleteOldGUI then
		for i,v in pairs(game.CoreGui:GetChildren()) do
			if v.Name == "hublin" then v:Destroy(); end
	...
	if gameID and gameID ~= game.PlaceId then
		...
		local notif = hublin.newNotif(hublin.enum.notifType.buttons, "Incompatible game for GUI. Continue anyway?", "Yes", "No",
	...
	mainGUI.closenav.MouseButton1Click:Connect(function() if game:GetService("CoreGui"):FindFirstChild("hublin") then
	           game:GetService("CoreGui")["hublin"]:Destroy();
	       end
	end)
	-- For toggling
	game:GetService("UserInputService").InputBegan:Connect(function(input)
